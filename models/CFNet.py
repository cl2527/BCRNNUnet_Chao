...
class outconv(nn.Module):
    def __init__(self,output_channel=2):
        ...
        self.conv = nn.Conv2d(64, output_channel, 1)
    def forward(self, x):
        x = self.conv(x)
        return x
...
class FE(nn.Module):
    def __init__(self, input_channel=3):
        ...
        self.fe = nn.Sequential(
            nn.Conv2d(input_channel, 64 ,3, padding=1), nn.PReLU(),
...
# -----------deep feature extractor Deep_FE--------------------------
...
# ----------integrate noise estimatro FCN + SFT --------------------
...
class CFNet(nn.Module):
    def __init__(self,input_channel=64, output_channel=2):
        super(CFNet, self).__init__()
        self.endconv = outconv(output_channel)
        self.FE1 = FE(input_channel)
        self.conv1 = nn.Sequential(nn.Conv2d(input_channel,64,1),nn.PReLU())
        # -------stage1----------------
